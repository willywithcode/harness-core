...
`CONTEXT.md` sits at the root of each context, next to that context's
decision records. Read [ADR-FORMAT.md](ADR-FORMAT.md) first: if the repo
already has a decision-record convention (`docs/decisions/`, or whatever
it actually uses), decisions go there, not in a new `docs/adr/`. The tree
below is only the fallback layout, used when no such convention exists yet.
...
Create files lazily: only when you have something to write. If no `CONTEXT.md` exists, create one when the first term is resolved. If no decision directory exists yet (see above), create it when the first ADR is needed.
